...
def execute(filters=None):
	...
	for cl in sl_entries:
		...
		count_packed = frappe.db.sql("""select count(*) from `tabPacked Item` where parent = %s""", cl.name)[0][0]
		...
		for q in range(0,count):
			...
			if count_so > q:
				...
				y = flt(y)+1
				items = frappe.db.sql("""select `name` from `tabPacked Item` where parent = %s order by idx asc limit %s,%s """, (cl.name, x, y))[0][0]
				det = frappe.db.get_value("Packed Item", items, ["item_code", "description", "qty"], as_dict=1)
				x = flt(x)+1
			...
			check_actual = frappe.db.sql("""select count(*) from `tabStock Ledger Entry` where item_code = %s and warehouse = %s""", (det.item_code, filters.get("warehouse")))[0][0]
			if check_actual != 0:
