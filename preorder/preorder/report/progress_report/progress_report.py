	...
	for cl in sl_entries:
		...
		for q in range(0,count):
			...
			if flt(q) < flt(count_quote):
				quote = frappe.db.sql("""select distinct(qo.`name`) from `tabQuotation Item` qi inner join `tabQuotation` qo on qi.parent = qo.`name` where qi.inquiry = %s and qo.docstatus != '2' order by qo.`name` limit %s,%s""",(cl.name, q, i))[0][0]
			...
			if flt(q) < flt(count_so):
				so = frappe.db.sql("""select distinct(so.`name`) from `tabSales Order Item` soi inner join `tabSales Order` so on soi.parent = so.`name` where soi.inquiry = %s and so.docstatus != '2' order by so.`name` limit %s,%s""",(cl.name, q, i))[0][0]
